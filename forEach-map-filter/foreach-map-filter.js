...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(currentString){
       result.push(currentString[0] + currentString[currentString.length -1])
    });
    return result;
}
...
function vowelCount(str){
    //store the string, spilt up the letters, this turns it into an sub array of individual letters split with " ".
        let splitWord = str.split("");
     //make an empty object to store the letter count in    
        let obj = {};
        //make a const to store the vowels so you can compare them to each letter in the string provided
        const vowels = 'aeiou'

    //on the split string call forEach
    //in the callback function set the parameter to "letter"
            splitWord.forEach(function(letter){
            //make sure to set a variable to store the lowercased letters in the sub array.  
            let lowerCasedLetter = letter.toLowerCase(); 
            //check if the sub array has any vowels by using indexOf method on vowels variable, in indexOf
            //put the lowerCasedLetter variable into it and check if there are any vowels
...
Write a function called find which accepts an array and a value and returns the first
 element in the array that has the same value as the second parameter or undefined if the value is not found 
 in the array.
